...
iter_max = 70000;       %maximum number of iterations
...
for iter = 1:iter_max
    ...
    for i = 2:n-1
        for j = 2:n-1
            ...
            Tt(i,j) = (4*(Tp(i+1,j) + Tp(i-1,j) + Tp(i,j+1) + Tp(i,j-1)) + (Tp(i+1,j+1) + Tp(i-1,j-1) + Tp(i+1,j-1) + Tp(i-1,j+1)))/20;
